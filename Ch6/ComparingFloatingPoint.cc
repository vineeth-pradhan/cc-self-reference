...
bool ComparingFloatingPoint::absApproximation(float a, float b, float absEpsilon)
{
  return std::abs(a-b) <= absEpsilon;
}

// irb> (90000.89843750 - 90000.90625000).abs <= ([90000.89843750, 90000.90625000].max * 0.00001)
bool ComparingFloatingPoint::relApproximation(float a, float b, float relEpsilon)
{
  // If close to 0, return true. Otherwise use relative weightage
  if(ComparingFloatingPoint::absApproximation(a, b, relEpsilon))
    return true;

  return std::abs(a-b) <= (std::max(std::abs(a),std::abs(b)) * relEpsilon);
}

void ComparingFloatingPoint::execute()
{
